//크로스 브라우져랑 코드가 간결해져서 쓸만하다. jQuery는...
function bit(value) {
	var obj;
	if(value instanceof HTMLElement) {
		...
		obj= [document.createElement(value.substr(1,value.length-2))];
		//e= document.createElement(value.substring(1,value.length-1));
	}else{
...
//			button, relatedTarget);
		...
		for(var i=0;i<this.length; i++) {
			if(this[i].addEventListener) {//크롬, 사파리, 파폭, IE
