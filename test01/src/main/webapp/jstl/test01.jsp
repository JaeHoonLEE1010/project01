<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    <%-- JSTL 사용 준비와 데이터 출력하기 
      1)출력: <c:out> 태그
      <c:out value="출력할 데이터" default="기본 데이터"/>
      <c:out value="출력할 데이터"/>기본 데이터 </c:out>
      2)변수 만들기:<c:set> 태그
      <c:set var="변수명" value="값" scope="page|request|session|appication"/>
      <c:set var="변수명" scope="page|request|session|appication">값</c:set>
    --%>
     <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<!DOCTYPE html>
<html>
...
<%--보관소에 등록된 객체를 삭제한다.
  var="삭제할 객체명"
  scope="page|request|session|appication"
 --%>
 <h2>삭제 전</h2>
 name=${name }<br>
 age=${age }<br>
